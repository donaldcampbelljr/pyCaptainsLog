...
def main():
    ...
    initial_input = (
        f"Let us roleplay. I am a Captain of the {ship.name} on a mission of combat in the System {planet_name}."
    ...
    while player_input != "leave":
        ...
        layout["middle"].split_row(
            Layout(name="middleleft"),
            Layout(name="middleright"),
        )

        layout["middleleft"].ratio = 2

        layout["middleright"].split_column(
            Layout(name="Health"), Layout(name="EnemyHealth")
        )
        ...
        if "name" in planet_dict:
            upper_text.append(
                f"You've entered into orbit of the planet {planet_name}", style="cyan3"
            )
        ...
        ship_attributes_text = Text(
            f"STRENGTH: {ship.strength}\nSCIENCE: {ship.science}\nDIPLOMACY: {ship.diplomacy}"
        )
        ...
        try:
            ...
            if sel <= len(item_titles):
                ...
                response = get_gemini_reponse(
                    f"I play this card: {item_titles[sel]} with info {item_descs[sel]} and power {item_type[sel]} {item_power[sel]}. Narrate how this affects the battle."
                )
                event_text = response.text
...
def generate_cards_dumb():
    ...
    items = {}
    items.update(
        {"Biomod Printer": "Prints and installs custom biological modifications."}
    )
    items.update(
        {
            "Zero-Point Converter": "Harnesses energy between dimensions, granting temporary power with unpredictable consequences."
        }
    )
    items.update(
        {
            "Sentient Holotape": "Data storage containing a digitized consciousness, eager to share forbidden knowledge."
        }
    )
    items.update(
        {
            "Stasis Grenade": "Suspends objects and people in frozen time for tactical escapes or interrogations."
        }
    )
    items.update(
        {
            "Neural Weaver": "Helmet for telepathic communication and manipulation, risking permanent consciousness merging."
        }
    )
    items.update(
        {
            "Gene Weaver Serum": "Grants temporary access to alien abilities at the cost of potential side effects."
        }
    )
    items.update(
        {
            "Quantum Compass": "Points to probabilities or potential futures, guiding through uncertain choices."
        }
    )
